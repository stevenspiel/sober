...
describe Sober do
  let(:default_sentence){Sober.sentence}
  ...
  describe '#sentence' do
    let(:short_sentence){Sober.sentence(:short)}
    let(:medium_sentence){Sober.sentence(:medium)}
    let(:long_sentence){Sober.sentence(:long)}
  ...
  describe '#sentences(n)' do
    it 'returns the number of sentences (periods) defined in the argument' do
      expect(Sober.sentences(2).count('.')).to eq(2)
      expect(Sober.sentences(5).count('.')).to eq(5)
    end
  end

  describe '#paragraph' do
    let(:default_paragraph){Sober.paragraph}
    let(:short_paragraph){Sober.paragraph(:short)}
    let(:medium_paragraph){Sober.paragraph(:medium)}
    let(:long_paragraph){Sober.paragraph(:long)}
  ...
  describe '#paragraphs(n)' do
    it 'separates paragraphs with a blank line' do
      expect(Sober.paragraphs.match(/\n\n/)).to be_true
    end

    it 'returns more than one paragraph' do
      expect(Sober.paragraphs.scan(/\n\n/).size).to be >= 1
    end
    ...
    it 'returns the number of paragraphs in the argument' do
      expect(Sober.paragraphs(10).scan(/\n\n/).size).to be == 9
    end
  end
